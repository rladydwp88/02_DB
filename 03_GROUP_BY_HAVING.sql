...
-- EMPLOYEE 테이블에서 (FROM 절)
-- 부서코드가 D5, D6인 부서의 (WHERE 절 -> GROUP 절)
-- 부서코드, 평균급여, 인원 수 조회 (SELECT 절)
SELECT DEPT_CODE 부서, ROUND(AVG(SALARY)) 평균급여, COUNT(*) 인원수
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서
-- 부서별로 같은 직급인 사원의 인원 수를 조회 (GROUP DEPT_CODE, JOB_CODE 묶음)
...
-- CUBE : CROUP BY절에 작성된 모든 컬럼의 중간 집계를 처리하는 함수
SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE
...
WHERE DEPT_CODE = 'D5'
MINUS
-- (2번째 SELECT문)급여가 300만원 초과인 사원 사번, 이름, 부서코드, 급여 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
