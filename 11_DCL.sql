...
CREATE USER kyj_sample IDENTIFIED BY 1234;
...
GRANT CREATE SESSION TO kyj_sample;
...
-- CREAE TABLE : 테이블 생성 권한
-- + 데이터를 저장할 수 있는 공간(TABLESPACE) 할당

-- 6.(sys) 테이블 생성 권한 부여 + TABLESPACE 할당
GRANT CREATE TABLE TO kyj_sample;

ALTER USER kyj_sample DEFAULT
...
--> CREATE TABLE, CREATSEQUENCE,..

-- (sys) sample 계정에 CONNECT, RESOURCE ROLE 부여
GRANT RESOURCE, CONNECT TO kyj_sample;
...
GRANT SELECT ON kh.EMPLOYEE TO kyj_sample;
...
REVOKE SELECT ON kh.EMPLOYEE FROM kyj_sample;
