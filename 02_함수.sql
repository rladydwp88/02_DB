...
-- 단일 행 함수 : N개 값을 읽어서 연산 후 N개의 결과를 반환
...
SELECT INSTR('AABAACAABBAA', 'B', 5, 2) FROM DUAL; -- 910

-- EMPLOYEE 테이블에서 사원명, 이메일, 이메일 중 '@' 조회
...
-- EMPLOYEE 체이블에서 사원명, 이메일 중 아이디만 조회
-- sun_di@or.kr -> sun_di
...
-- EMPLOYEE 테이브에서 사원의 월급을 100만으로 나눴을 때 나머지 조회
...
SELECT TRUNC(123.456) FROM DUAL; -- 123, 소수점 아래를 첨삭
...
-- SYSDATE(상수) : 시스템에 현재 시간(년, 월, 일, 시, 분, 초)을 반환
SELECT SYSDATE FROM DUAL; -- 2025-03-10 12:11:16.000

-- SYSTIMESTAMP(상수) : SYSDATE + MS 단위 추가
SELECT SYSTIMESTAMP FROM DUAL; --2025-03-10 12:12:24.508 +0900

-- MONTHS_BETWEN(날짜, 날짜) : 두 날짜의 개월 수 차이 반환
...
-- ADD_MONTH(날짜, 숫자) : 날짜에 숫자만크의 개월 수를 더함(음수도 가능)
SELECT ADD_MONTHS(SYSDATE, 4) FROM DUAL; -- 4개월 더함
SELECT ADD_MONTHS(SYSDATE, -1) FROM DUAL; -- 4개월 뺌
...
-- 형변환 함수
-- 문자열(CHAR), 숫자(NUMBER), 날짜(DATE) 끼리의 형변환 가능
...
SELECT TO_CHAR(1000000, 'L9,999,999') || '원' "원화" FROM DUAL;
...
-- 2025년 3월 10일 (월)
SELECT TO_CHAR(SYSDATE, 'YYYY / MM 월 / DD 일 (DY)') FROM DUAL;

SELECT EMP_NAME, JOB_CODE, SALARY,
DECODE(JOB_CODE, 'J7', SALARY*1.2) "인상된 급여"
FROM EMPLOYEE;
...
-- 그룹 함수
-- 그룹 함수 하나 이상의 행을 그룹으로 묶어 연산하여 총합, 평균 등의
-- 하나의 결과 행으로 반환하는 함수
...
-- CONUNT([DISTINCT] 컬러명) : 중복을 제거한 행 개수를 리턴
