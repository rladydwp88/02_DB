...
--> 상관 서브쿼리를 사용하여 각 직원에 MANAGER_ID가
...
-- D8부서의 가장 빠른 입사일
-- 메인쿼리에서 퇴사자 & D8부서의 가장빠른 입사일인 이태림을 제외시킨 상태
...
FROM (SELECT EMP_NAME, SALARY
			FROM EMPLOYEE
			ORDER BY SALARY DESC)	-- 해석 순위 1위 FROM 절에서 전체 직원의 SALARY 내림차순으로 조회
WHERE ROWNUM <= 5 -- 해석 순위 2위 WHERE절에서 가상컬럼의 1~5행까지만 조회
ORDER BY SALARY DESC;
...
-- 8. WITH
-- 서브 쿼리에 이름을 붙여주고 사용 시 이름을 사용하게 함
